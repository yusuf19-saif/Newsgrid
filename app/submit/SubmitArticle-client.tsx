app/submit/SubmitArticleClient.tsx
'use client';
...
const SubmitArticleClient = ({ categories }: SubmitArticleClientProps) => {
  ...
  const parseAiResponse = (text: string) => {
    ...
    if (!text.trim().startsWith('## NewsGrid AI Article Review')) {
      parsed.raw = text;
      return parsed;
    }
    ...
    const extractContent = (sectionText: string) =>
      sectionText.substring(sectionText.indexOf('\n') + 1).trim();
    ...
    if (scoreMatch?.[1]) parsed.confidenceScore = parseInt(scoreMatch[1], 10);

    parsed.suggestedImprovements = extractContent((sections[6] || '').split('---')[0] || '');
  ...
  const handleSubmit = async (status: 'draft' | 'pending_review') => {
    ...
    try {
      ...
      if (status === 'pending_review') {
        ...
        if (!slugToSave) throw new Error('Invalid headline. Please provide a more descriptive headline to submit for review.');
      } else {
      ...
      console.error('[Submit] An error occurred during submission:', err);
      if (err.code === '23505') {
            ...
            <div key={analysisKey} className={styles.aiReport}>
              ...
              {verificationResult.raw ? (
                <div className={styles.reportSection}>
                  ...
                  <p>The AI response did not match the expected format. Displaying the raw output:</p>
                  <pre style={{ whiteSpace: 'pre-wrap', background: '#222', padding: '1rem', borderRadius: 8 }}>
                    {verificationResult.raw}
                  </pre>
                </div>
              ) : (
                <>
                  {verificationResult.confidenceScore !== null && (
                    <div className={styles.trustScoreSection}>
                      <TrustScoreMeter score={verificationResult.confidenceScore} />
                    </div>
                  )}
                  ...
                  {verificationResult.claimByClaimSupport && (
                    <div className={styles.reportSection}>
                  ...
                  {verificationResult.missingEvidence && (
                    <div className={styles.reportSection}>
                  ...
                  {verificationResult.sourceQualityAssessment && (
                    <div className={styles.reportSection}>
                  ...
                  {verificationResult.confidenceScore !== null && (
                    <div className={styles.reportSection}>
                      ...
                      <p>{verificationResult.confidenceScore}%</p>
                    </div>
                  )}

                  {verificationResult.suggestedImprovements && (
                    <div className={styles.reportSection}>
